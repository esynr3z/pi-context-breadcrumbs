...
export function normalizeConfig(raw) {
    ...
    return {
        ...
        filterSupersededFromPrompt: typeof input.filterSupersededFromPrompt === "boolean"
...
export function loadConfig(cwd, notify) {
    let merged = {};
...
function collectPathValues(value, parentKey, out) {
    if (typeof value === "string") {
        if (parentKey && isPathKey(parentKey))
            out.push(value);
        return;
    }
    if (Array.isArray(value)) {
        for (const item of value)
            collectPathValues(item, parentKey, out);
        return;
    }
...
export function extractFilesystemPaths(toolName, input, isBuiltinTool = true) {
    if (toolName === "bash" || toolName === "shell")
        return [];
    const fsToolNames = new Set(["read", "write", "edit", "ls", "list", "grep", "search", "find"]);
    if (!isBuiltinTool && !fsToolNames.has(toolName))
        return [];
    const out = [];
...
function hasIgnoredSegment(relativePath, ignoreDirs) {
...
function isGitIgnored(gitRoot, absPath) {
    if (!isUnderPath(absPath, gitRoot))
        return false;
...
function nearestExistingAncestor(filePath, stopAt) {
...
function formatAppliesTo(dirRel) {
...
export class NestedContextManager {
    ...
    ignoreDirSet;
    loaded = new Map();
    observedDirs = new Set();
    startupContextPaths = new Set();
    warned = new Set();
    constructor(cwd, config = DEFAULT_CONFIG, notify) {
    ...
    setStartupContextFiles(files) {
        ...
        for (const file of files ?? []) {
            ...
            if (realPath)
                this.startupContextPaths.add(path.normalize(realPath));
        }
    ...
    normalizeObservedTarget(rawPath) {
        const dir = this.resolveTargetDirectory(rawPath);
        if (!dir)
            return undefined;
        ...
        if (!isUnderPath(resolved, this.cwdAbs))
            return undefined;
    ...
    resolveTargetDirectory(rawPath) {
        const cleaned = stripAtPrefix(rawPath);
        if (!cleaned)
            return undefined;
        const resolved = path.normalize(path.isAbsolute(cleaned) ? path.resolve(cleaned) : path.resolve(this.cwdAbs, cleaned));
        ...
        try {
            ...
            if (!realTarget || !isUnderPath(path.normalize(realTarget), this.cwdReal))
                return undefined;
            if (lst.isDirectory()) {
    ...
    discoverForDirectory(targetDir, options = { notifyFirstLoad: true }) {
    ...
    loadCandidate(absPath, includeIndex, options) {
        ...
        try {
            if (!isUnderPath(normalizedAbs, this.cwdAbs))
                return invalidate();
            if (this.isIgnoredPath(normalizedAbs))
                return invalidate();
            if (this.startupContextPaths.has(normalizedAbs))
                return invalidate();
            let lst;
            try {
            ...
            if (!realPath || !isUnderPath(path.normalize(realPath), this.cwdReal))
                return invalidate();
            if (this.startupContextPaths.has(path.normalize(realPath)))
                return invalidate();
...
function getBuiltinToolNames(pi) {
    try {
        return new Set(pi
            .getAllTools()
...
function formatList(entries) {
    if (entries.length === 0)
        return ["Context breadcrumbs: no files loaded."];
    return [
...
export default function contextBreadcrumbsExtension(pi) {
    ...
    pi.on("context", async (event) => {
        try {
            if (!manager?.config.enabled || !manager.config.filterSupersededFromPrompt)
                return;
